...
import io.hoistapp.user.UserService
...
class AuthenticationService extends BaseAuthenticationService  {
    ...
    protected boolean completeAuthentication(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!tokenResult.isValid) {
            logInfo("Invalid token result - user will not be installed on session - return 401", tokenResult.exception)
            return true
        }
		
        def user = userService.getOrCreateFromJwtResult(tokenResult)
        setUser(request, user)
        logInfo("User read from token and set on request", "username: ${user.username}")
