...
class DBConfig {
    ...
    static void dataSourceConfig(Script script) {
        ...
        } else if(getInstanceConfig('dbType') == 'mssql'){
            msSqlConfig(script)
        }
        else if(getInstanceConfig('dbType') == 'mysql'){
            mySqlConfig(script)
        }
    }

    static void msSqlConfig(Script script) {
        withDelegate(script) {
            //---------------------
            // Datasource
            //---------------------
            def dbHost = getInstanceConfig('dbHost'),
                dbSchema = getInstanceConfig('dbSchema'),
                dbDomain = getInstanceConfig('dbDomain') ?: 'elliottmgmt.com'
            ...
            dataSource {
                ...
                dbCreate = getInstanceConfig('dbCreate') ?: 'update'

                // jTDS specific driver params are passed via the JDBC URL. Note timeouts are in seconds.
                // The socketTimeout must be longer than the max expected query time, but as per jTDS FAQ docs
                // specifying it "may be useful for detecting dead network connections in a pooled environment".
                url = "jdbc:jtds:sqlserver://$dbHost:1433/$dbSchema;domain=$dbDomain;useNTLMv2=true;loginTimeout=15;socketTimeout=180"
                ...
                properties {
                    ...
                    // ...meaning those in use longer than this value (must be longer than max query time)...
                    removeAbandonedTimeout = 3 * MINUTES
                    // ...but don't necessarily abandon until we actually need them freed up.
    ...
    private static void mySqlConfig(Script script) {
        withDelegate(script) {
            ...
            // hoistapp uses mySQL, but apps can use any DB supported by Grails (SQL Server, Postgres, etc).
            //----------------------------------------------------------------------------------------------
